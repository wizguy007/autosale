<?php
  ...
  $odid = $_GET['order_id'];
  $sql = "SELECT * FROM `booking` WHERE `booking_id`='".$odid."'";
  $query = mysqli_query($db , $sql);
  $fetch = mysqli_fetch_assoc($query);

  $booking_id_id = $fetch['booking_id'];
...
<section id="main">
  ...
  <div>
    <div id="container" class="target" style="background: #ebebe0;margin-bottom:20px;margin-top:20px;min-height:400px;max-width: 820px;box-shadow:0px 0px 3px 3px rgba(12,13,14,0.15);" >
      <section id="memo">
        <div class="company-name">
          <span>AutoSale<small style="font-size: 14px;margin-top: 5px;margin-left: 5px;"> Rent</small></span>
        </div>
